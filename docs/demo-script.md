...
| 加分项 | RAG、MCP、Streaming、可观测性、单元测试 |
...
项目包含 29 个 xUnit 测试，覆盖 Agent Loop、工具调用、记忆、RAG、本地 embedding、流式 tool call、中文输入编辑、课件精读、MCP 工具、学习画像、画像补参兜底和复习计划。测试使用 `FakeLlmClient`，不依赖真实 LLM API，因此可稳定运行。
