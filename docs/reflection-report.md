...
本项目的目标是完成一个基于 .NET 8 的 AI Agent：SmartStudy 智能学习助手。它面向《.NET 体系结构设计与开发》课程复习场景，用户可以用自然语言询问 Agent 相关概念，例如 ReAct、Semantic Kernel、MCP、Agent Framework 等。Agent 不只是一次性回答，而是会根据任务需要自主决定是否检索课程资料、保存学习笔记、维护学习画像、生成复习计划、列出已有笔记、生成练习题或进行简单计算。
...
| `SmartStudy.Mcp` | MCP stdio Server，把笔记、知识库检索和课程资料清单暴露给外部 MCP 客户端 |
...
本项目实现了 17 个工具，超过课程要求的 3 个。

| 工具名 | 文件 | 设计目的 |
| --- | --- | --- |
| `knowledge_search` | `KnowledgeSearchTool.cs` | 对课程资料做语义检索，支持 RAG |
| `read_course_material` | `ReadCourseMaterialTool.cs` | 按文件名读取已导入课件的连续页内容，解决用户要求精读某个 PDF 时只返回碎片的问题 |
| `import_course_materials` | `ImportCourseMaterialsTool.cs` | 读取本地课程资料单文件或目录，抽取 PDF/PPTX/DOCX/XLSX/CSV/TSV/HTML/TXT/MD 文本并重建 RAG 索引 |
| `add_note` | `NoteTools.cs` | 保存学习笔记到 JSON 文件 |
| `list_notes` | `NoteTools.cs` | 按标签或关键字查询笔记 |
| `update_learning_profile` | `LearningProfileTools.cs` | 更新长期学习画像，包括薄弱项、优势项、目标和讲解偏好 |
| `show_learning_profile` | `LearningProfileTools.cs` | 查看当前学习画像 |
| `study_plan` | `LearningProfileTools.cs` | 根据学习画像和课程资料生成复习计划 |
| `add_study_task` | `StudyProgressTools.cs` | 添加可追踪的学习任务 |
| `mark_task_done` | `StudyProgressTools.cs` | 标记学习任务完成并记录复盘 |
| `show_progress` | `StudyProgressTools.cs` | 查看完成率、累计学习时长和待办任务 |
| `review_history` | `StudyProgressTools.cs` | 查看最近学习历史和复盘记录 |
| `make_quiz` | `MakeQuizTool.cs` | 基于材料生成练习题，隐藏答案，并对 JSON 结构进行提取、校验和修复 |
| `submit_quiz_answer` | `QuizResultTools.cs` | 根据练习编号和题号判分，判分后展示答案和解析 |
| `record_quiz_result` | `QuizResultTools.cs` | 手动记录答题结果，错题自动同步到学习画像薄弱项 |
| `show_mistakes` | `QuizResultTools.cs` | 查看错题本、正确率和错因解析 |
...
当前短期对话记忆在 CLI / Web 中由 `ProjectConversationMemory` 管理，按 `data/web-projects/<project-id>/data/memory/<conversation-id>.json` 持久化，切换项目或对话时恢复对应历史；内存版 `ConversationMemory` 仍用于单元测试和简单场景。长期知识由项目级 RAG 索引承担，学习笔记由 `JsonNoteStore` 持久化到当前项目的 `notes.json`，学生画像由 `JsonLearningProfileStore` 持久化到当前项目的 `learning-profile.json`。学习画像保存薄弱知识点、优势知识点、学习目标和偏好讲解风格，`study_plan` 工具会读取这些信息并结合知识库检索结果生成复习安排。
...
`SmartStudy.Mcp` 使用官方 `ModelContextProtocol` C# SDK 实现 stdio Server，并暴露四个工具：

| MCP 工具 | 功能 |
| --- | --- |
| `AddNote` | 保存一条学习笔记 |
| `ListNotes` | 查询笔记 |
| `SearchKnowledge` | 复用 `KnowledgeSearchService` 在课程知识库中检索相关片段 |
| `ListImportedMaterials` | 复用 `CourseMaterialCatalog` 列出已导入的课程资料文件 |

这样做的意义是证明同一套领域能力不仅可以被本项目 CLI 使用，也可以通过 MCP 被其他 Host 使用，例如 Claude Desktop、VS Code MCP Client 或其他兼容客户端。MCP Server 启动时会读取配置并加载已有 RAG 索引；如果 MCP 输出目录没有索引，会尝试复用 CLI 输出目录中的索引。
...
CLI 还增加了 `doctor` / `status` / `tools` 诊断命令。`SmartStudyDoctor` 会读取强类型配置、LLM profile、Embedding provider、RAG 索引文件、知识库目录、笔记文件和工具注册中心，生成一个可展示的健康检查快照。这样现场演示前可以先确认项目是否具备演示条件，也方便答辩时说明当前系统真实加载了哪些工具和数据。
...
6. 后续迭代中补充了中文输入光标宽度处理、指定课件精读工具、`make_quiz` 的结构化 JSON 校验和隐藏答案流程、`submit_quiz_answer` 判分工具，以及长期学习画像与复习计划工具。
...
| ReadCourseMaterialTool | 按文件名和页码读取已导入课件 |
| MakeQuizTool | JSON 提取、结构校验、修复重试、失败提示 |
| SmartStudyDoctor | 汇总 LLM、Embedding、RAG、笔记和工具注册状态 |
...
5. MCP Server 已暴露笔记、RAG 检索和课件列表能力，后续可以继续扩展为完整的 MCP 学习助手工具集。
