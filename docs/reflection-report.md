...
| `make_quiz` | `MakeQuizTool.cs` | 基于材料生成练习题，并对 JSON 结构进行提取、校验和修复 |
...
6. 后续迭代中补充了中文输入光标宽度处理、指定课件精读工具、`make_quiz` 的结构化 JSON 校验和失败重试，以及长期学习画像与复习计划工具。
...
这些测试不依赖真实 LLM API，而是使用 `FakeLlmClient` 构造固定响应，因此可以稳定验证 Agent 主循环。当前测试结果为 29/29 通过。
