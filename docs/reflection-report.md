...
| `import_course_materials` | `ImportCourseMaterialsTool.cs` | 读取本地课程资料目录，抽取 PDF/PPTX/DOCX/TXT/MD 文本并重建 RAG 索引 |
...
当前短期对话记忆由 `ConversationMemory` 管理。长期知识由 RAG 索引承担，学习笔记由 `JsonNoteStore` 持久化到 `notes.json`，学生画像由 `JsonLearningProfileStore` 持久化到 `learning-profile.json`。学习画像保存薄弱知识点、优势知识点、学习目标和偏好讲解风格，`study_plan` 工具会读取这些信息并结合知识库检索结果生成复习安排。
