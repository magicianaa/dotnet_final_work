...
| Tool Calling | 至少 3 个自定义工具 | 已完成，当前注册 10 个工具 |
...
| Tools | 已有检索、精读资料、导入资料、笔记、学习画像、复习计划、出题、计算 | 后续可继续增加模型连通性自检和学习进度统计 |
| RAG | 支持云端 embedding 与本地 Hash Embedding | 可以增加索引统计与来源列表命令 |
| CLI | 支持中文行内编辑、流式输出、模型切换 | 可以增加 `/help` 或命令帮助面板 |
| MCP | 暴露笔记、知识库检索和资料清单 MCP 工具 | 可以继续扩展学习画像 MCP 工具 |
| Tests | 29 个测试覆盖核心逻辑 | 继续覆盖新功能和边界情况 |
...
目标：让 `make_quiz` 不只是“要求模型输出 JSON”，而是由 C# 代码实际校验 JSON 结构。如果模型输出 Markdown、解释文字或字段缺失，工具应自动发起修复请求；如果仍失败，应返回可读错误。
