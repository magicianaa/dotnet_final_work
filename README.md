...
| 记忆机制 | `ProjectConversationMemory` 按项目 / 对话持久化短期上下文，`JsonNoteStore` 持久化学习笔记，`JsonLearningProfileStore` 持久化长期学习画像 |
| RAG | `JsonPersistentVectorStore` 将项目级向量索引持久化到 `data/web-projects/<project-id>/data/index.json`，支持智谱 `embedding-3` 云端向量化和完全离线的本地 Hash Embedding；检索结果包含来源、ChunkId、相似度和来源汇总 |
...
flowchart LR
    ...
    Import --> Imported["project knowledge/imported"]
    Read --> Imported
    Notes --> NoteFile["project data/notes.json"]
    Profile --> ProfileFile["project data/learning-profile.json"]
    Plan --> ProfileFile
    Plan --> RAG
...
| `import_course_materials` | 读取本地资料文件或目录并导入知识库 | “我的课程资料在某个文件/文件夹，请阅读它们” |
...
`doctor` / `status` 会显示当前 LLM profile、Embedding provider、知识库目录、RAG 索引、向量库类型、持久化路径、笔记文件、学习画像文件和工具注册状态。`tools` 会列出所有 Agent 工具。
...
| `:project delete <项目ID或名称>` | 删除学习项目，并同步删除该项目的资料、索引、笔记、画像、进度、错题、对话和记忆 |
| `:conversations` | 列出当前项目下的学习对话 |
| `:conversation new <标题>` | 在当前项目中新建学习对话 |
| `:conversation switch <对话ID或标题>` | 切换当前项目下的学习对话 |
| `:conversation delete <对话ID或标题>` | 删除当前项目下的学习对话，并删除对应短期记忆文件 |
...
dotnet run --project src\SmartStudy.Cli\SmartStudy.Cli.csproj -- project delete 4f9e71ef4b
...
dotnet run --project src\SmartStudy.Cli\SmartStudy.Cli.csproj -- conversation delete "期末复习"
```

项目数据默认保存在 `data/web-projects/<project-id>/`。每个项目拥有自己的 `knowledge/`、`data/index.json`、笔记、学习画像、进度、错题、练习会话，以及 `data/memory/<conversation-id>.json` 对话记忆。切换项目或对话会重新加载对应记忆，不会清空历史；只有 `:reset`、Web 中的“清空”或模型切换才会重置当前对话上下文。
...
| `:import <path> \| <glob>` | `import_course_materials` | `:import C:\Course\PPT \| Lesson00` 或 `:import C:\Course\Chp6TestTool.pdf` |
...
当用户给出本地文件或文件夹并要求“阅读 / 导入课程资料”时，Agent 应调用 `import_course_materials`，而不是回答“无法访问文件”。工具会完成以下步骤：

1. 接受单个资料文件，或扫描目录中的 `.pdf/.pptx/.docx/.xlsx/.csv/.tsv/.html/.htm/.md/.txt`
2. 抽取文本
3. 写入当前学习项目的 `knowledge/imported`
4. 重建当前项目的 RAG 索引
...
4. 导入成功后，当前项目的 `knowledge/imported` 中生成对应 `.md` 文件，并自动重建 RAG 索引。
5. 单元测试 `CourseMaterialImporterExtendedFormatTests.ImportsSingleMaterialFile` 和 `CourseMaterialImporterExtendedFormatTests.ImportsCsvHtmlAndXlsxMaterials` 通过。

注意：CLI 与 Web 共用仓库根目录下的项目工作区。项目级数据通常位于：

```text
data/web-projects/<project-id>/data/
data/web-projects/<project-id>/knowledge/imported/
```

发布运行后，数据位于发布目录下的项目工作区。学习笔记、长期学习画像、学习进度、错题和对话记忆均按项目隔离保存。
