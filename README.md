...
SmartStudy 是一个使用 C# / .NET 8 实现的控制台 AI Agent。它面向课程学习场景，支持用户用自然语言提问课程资料、导入本地课件、精读某个 PDF/PPTX/DOCX 文件、保存学习笔记、维护长期学习画像、生成复习计划、生成复习题、进行简单计算，并通过 ReAct 循环自主选择是否调用工具。
...
| Tool Calling | 当前注册 10 个自定义工具，覆盖检索、导入、文件精读、笔记、学习画像、复习计划、出题和计算 |
| 记忆机制 | `ConversationMemory` 管理短期上下文，`JsonNoteStore` 持久化学习笔记，`JsonLearningProfileStore` 持久化长期学习画像 |
| RAG | 支持智谱 `embedding-3` 云端向量化，也支持完全离线的本地 Hash Embedding |
| 本地资料读取 | 支持导入 `.pdf/.pptx/.docx/.md/.txt` 到本地知识库 |
...
| `make_quiz` | 基于材料生成练习题，并校验 / 修复 JSON 结构 | “基于这段内容出 3 道选择题” |
